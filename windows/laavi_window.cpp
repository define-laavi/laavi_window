...
namespace laavi_window {
    ...
    BOOL is_laavi_window_loaded = FALSE;
    ...
    bool isLaaviWindowLoaded(){
        return is_laavi_window_loaded;
    }

    int init()
    {
        is_laavi_window_loaded = true;
    ...
    {
        has_custom_frame = (flags & LW_CUSTOM_FRAME);
        visible_on_startup = !(flags & LW_HIDE_ON_STARTUP);
        return 1;
    }
    ...
    void handle_lw_action(HWND window, WPARAM action, LPARAM actionData){
        switch (action)
        {
        case LW_SETWINDOWPOS:
        {
...
LRESULT CALLBACK main_window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam, UINT_PTR subclssID, DWORD_PTR refData)
{
    ...
    case WM_LW_ACTION:
    {
        handle_lw_action(window, wparam, lparam);        
        break;
    }
    default:
        break;
...
{
    return laavi_window::configure(flags);
}